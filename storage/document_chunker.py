...
class ContextEnrichedChunker:
    """Advanced chunker that preserves semantic coherence and enriches metadata."""
    ...
    def __init__(self,
        ...
        # Try loading spaCy for NER
        self.spacy = None
        try:
            import spacy
            self.spacy = spacy.load("en_core_web_sm")
        except (OSError, ImportError):
            print("spaCy model 'en_core_web_sm' not available. NER will be disabled.")

    def chunk_text(self, text: str, file_path: Path) -> List[DocumentChunk]:
        """Split text into semantically coherent, metadata-enriched chunks."""
        ...
        for idx, chunk_data in enumerate(raw_chunks):
            ...
            enhanced_metadata = {
                ...
                "has_questions": chunk_data.get('has_questions', False),
    ...
    def _analyze_document_structure(self, text: str) -> Dict[str, Any]:
        """Analyze document for headers, sections, and generate a summary."""
        structure = {
            'headers': [],
            'sections': [],
            'paragraphs': [],
        ...
        for i, line in enumerate(lines):
            ...
            if self._is_likely_header(stripped):
                structure['headers'].append({
                    'text': stripped,
                    'line_number': i,
                    'char_position': char_offset,
                    'level': self._estimate_header_level(stripped)
                })
    ...
    def _extract_semantic_units(self, text: str) -> List[Dict[str, Any]]:
        """Break text into sentence-level semantic units."""
        sentences = sent_tokenize(text)
        units = []
        char_offset = 0
    ...
    def _create_semantic_chunks(self, units: List[Dict], doc_structure: Dict) -> List[Dict[str, Any]]:
        """Group semantic units into chunks respecting size and boundary constraints."""
        chunks = []
        current_chunk = []
        current_size = 0
...
class DocumentChunker(ContextEnrichedChunker):
    """Simplified interface for context-enriched chunking."""
