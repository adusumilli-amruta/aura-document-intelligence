...
class QueryRequest(BaseModel):
    ...
    use_reranker: Optional[bool] = Field(False, description="Whether to use the reranker for refining results.")
...
class HybridSearchRequest(BaseModel):
    ...
    use_reranker: bool = Field(False, description="Whether to use the reranker for refining results.")
    reranker_top_k: Optional[int] = Field(10, description="Number of top results to return from the reranker.")
...
class WatcherBatchRequest(BaseModel):
    add_folders: Optional[List[str]] = None
    remove_folders: Optional[List[str]] = None
