...
class QueryService:
    """Central orchestration service for all document operations."""
    ...
    def folder_watcher(self):
        """Lazy-load folder watcher to avoid circular imports."""
    ...
    def get_random_chunks(self, limit: int = 20) -> List[SearchResult]:
        """Get random chunks for flashcard review."""
        try:
            points, _ = self.vector_storage.client.scroll(
                collection_name=self.vector_storage.collection_name,
                limit=limit,
                with_payload=True,
                with_vectors=False
            ...
            raise ServiceError(f"Failed to get random chunks: {str(e)}")
    ...
    def _build_filter(self, file_types: Optional[Union[str, List[str]]] = None,
                      file_names: Optional[Union[str, List[str]]] = None) -> Optional[Filter]:
        """Build Qdrant filter from file type and name criteria."""
        conditions = []
    ...
    def get_collection_info(self, collection_name: str = 'documents'):
        try:
            return self.vector_storage.get_collection_info()
        except Exception as e:
    ...
    def add_watch_folder(self, folder_path: str,
                         ...
                         recursive: bool = True,
                         auto_ingest: bool = True) -> Dict[str, Any]:
        try:
